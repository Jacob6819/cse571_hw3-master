...
def train_model(policy, baseline, trajs, policy_optim, baseline_optim, gamma=0.99, baseline_train_batch_size=64,
                baseline_num_epochs=5):
    # Fill in your policy gradient implementation here

    # TODO: Compute the returns on the current batch of trajectories Hint: Go through all the trajectories in trajs
    #  and compute their return to go: discounted sum of rewards from that timestep to the end. Hint: This is easy to
    #  do if you go backwards in time and sum up the reward as a running sum. Hint: Remember that return to go is
    #  return = r[t] + gamma*r[t+1] + gamma^2*r[t+2] + ...
    states_all = []
    actions_all = []
    returns_all = []
    for traj in trajs:
        ...
        # TODO start
        returns_singletraj[len(returns_singletraj) - 1] = rewards_singletraj[len(returns_singletraj) - 1]
        for i in range(len(returns_singletraj) - 2, -1, -1):
    ...
    # TODO start
    returns = torch.tensor((returns - returns.mean())/(returns.std() + 1e-9)).float().to(device)
    # TODO end
    ...
    for epoch in range(baseline_num_epochs):
        ...
        for i in range(n // baseline_train_batch_size):
            ...
            # TODO start         
            expert_ret_to_go = returns[batch_index]
            expert_states = torch.tensor(states).float().to(device)[batch_index]
            pred_rt_to_go = baseline(expert_states)
            loss = criterion(pred_rt_to_go, expert_ret_to_go).mean()
            # TODO end
    ...
    # TODO start
    loss = (-log_policy * (returns - baseline_pred)).mean()
    # TODO end
